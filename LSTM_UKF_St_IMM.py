...
class VolatilityRegimeSelector(tf.keras.layers.Layer):
    def __init__(self, num_regimes=3, history_window=100, learnable_centers=True, name="volatility_regime_selector"):
        ...
        self.temperature = self.add_weight(
            ...
            trainable=True,
            dtype=tf.float32,
            constraint=MinMaxClip(min_value=0.3, max_value=10.0)
        )
    ...
    # ✅ НОВЫЙ МЕТОД: вынесен на уровень класса
    @tf.function
    def _soft_quantile(self, x: tf.Tensor, q: float) -> tf.Tensor:
        """
        Мягкий квантиль через взвешенное среднее (дифференцируемо).
        Args:
            x: [H] — валидные значения волатильности
            q: процентиль (10.0, 50.0, 90.0)
        Returns:
            quantile: скаляр — вычисленный квантиль
        """
        # Мягкие веса на основе расстояния от медианы
        median = tf.reduce_mean(x)
        weights = tf.nn.softmax((x - median) / 0.1)  # [H]

        # Сортировка для порядка (градиент проходит через weights)
        sorted_idx = tf.argsort(x)
        sorted_x = tf.gather(x, sorted_idx)
        sorted_weights = tf.gather(weights, sorted_idx)

        # Взвешенное среднее около квантиля
        cumsum = tf.cumsum(sorted_weights)
        target = tf.cast(q, tf.float32) / 100.0
        mask = tf.cast(cumsum >= target, tf.float32)
        quantile = tf.reduce_sum(sorted_x * mask * sorted_weights) / (
            tf.reduce_sum(mask * sorted_weights) + 1e-8
        )
        return quantile
    ...
    def get_centers(self) -> tf.Tensor:
        ...
        def compute_adaptive():
            # ✅ ИСПРАВЛЕНО: вызов как метод класса
            q10 = self._soft_quantile(valid, 10.0)
            q50 = self._soft_quantile(valid, 50.0)
            q90 = self._soft_quantile(valid, 90.0)
    ...
    def get_regime_scales(self, soft_weights: tf.Tensor) -> tf.Tensor:
        ...
        # ✅ нормализация со stop_gradient
        regime_scale = regime_scale / (tf.stop_gradient(tf.reduce_mean(regime_scale)) + 1e-8)
...
class LSTMIMMUKF(tf.Module):
    ...
    def __init__(
        ...
        # === ЕДИНЫЕ КОНСТАНТЫ ИНИЦИАЛИЗАЦИИ ===
        self._init_cov_floor = 0.01
        self._init_cov_ceil = 0.5

        # Индексы признаков для быстрого доступа
        self.feature_indices = {name: idx for idx, name in enumerate(self.feature_columns)}
        ...
        with tf.device(self.device):
            ...
            # ✅ ИСПРАВЛЕНО: значение согласовано с комментарием (было 5.0, должно быть 1.0)
            self.Q_max_pred = tf.Variable(1.0, dtype=tf.float32, trainable=True, name='Q_max_pred')

            # === ДОБАВЛЕНО: ПАРАМЕТРЫ МАКСИМАЛЬНОЙ ШИРИНЫ ДОВЕРИТЕЛЬНОГО ИНТЕРВАЛА ПО РЕЖИМАМ ===
            self.max_width_factors_logits = tf.Variable(
                initial_value=tf.constant([1.0, 1.2, 1.5], dtype=tf.float32),   # более высокие начальные значения
                trainable=True,
        ...
        # coverage_mixing_alpha
        self.coverage_mixing_alpha = tf.Variable(
            initial_value=tf.constant(0.405, dtype=tf.float32),
            trainable=True,
        ...
        # ✅ ИСПРАВЛЕНО: синхронизация hyperparameters между оптимизаторами
        ...
        self._regime_optimizer = tf.keras.optimizers.Adam(
            ...
            beta_1=self._optimizer.beta_1,      # ← Синхронизировано
            beta_2=self._optimizer.beta_2,      # ← Синхронизировано
            epsilon=self._optimizer.epsilon     # ← Синхронизировано
        )
        ...
        # Инициализация индексов признаков для быстрого доступа в графовых функциях
        self.feature_indices = {}
        for idx, feature in enumerate(self.feature_columns):
            self.feature_indices[feature] = tf.constant(idx, dtype=tf.int32)
        ...
        # Инициализация _step_counter
        self._step_counter = tf.Variable(0, trainable=False, dtype=tf.int64, name="step_counter")

        # Инициализация _train_coverage_ema
        self._train_coverage_ema = tf.Variable(
            0.89, trainable=False, dtype=tf.float32, name='train_coverage_ema'
        )

        # ✅ ИСПРАВЛЕНО: добавить constraint для предотвращения неограниченного роста
        self.regime_calibration_weights = tf.Variable(
            initial_value=tf.constant([2.0, 3.0, 5.0], dtype=tf.float32),
            trainable=True,
            constraint=tf.keras.constraints.NonNeg(),  # ← Только неотрицательность
            name='regime_calibration_weights'
        )
    ...
    def _ensure_state_vars(self):
        ...
        if not hasattr(self, '_state_initialized'):
            self._state_initialized = tf.Variable(
                False, trainable=False, dtype=tf.bool, name="state_initialized"
            )
        ...
        if not hasattr(self, '_last_inflation_factor'):
            self._last_inflation_factor = tf.Variable(
                [1.0],
                trainable=False,
                dtype=tf.float32,
                name='last_inflation_factor'
            )
        if not hasattr(self, '_last_remaining_steps'):
            self._last_remaining_steps = tf.Variable(
                [0],
                trainable=False,
                dtype=tf.int32,
                name='last_remaining_steps'
            )
        if not hasattr(self, '_last_high_inflation_steps'):
            self._last_high_inflation_steps = tf.Variable(
                [0],
                trainable=False,
                dtype=tf.int32,
                name='last_high_inflation_steps'
            )
        if not hasattr(self, '_last_innov_window'):
            innov_window_size = getattr(self, 'innov_window_size', 20)
            self._last_innov_window = tf.Variable(
                tf.zeros([1, innov_window_size]),
                trainable=False,
                dtype=tf.float32,
                name='last_innov_window'
            )
        ...
        print("✅ Все переменные состояния инициализированы")
    ...
    def process_lstm_output(self, params_output):
        ...
        student_t_config = {
            ...
            'asymmetry_pos': 0.5 + 1.0 * tf.nn.sigmoid(student_t_params[..., 2:3]),    # макс 1.5
            'asymmetry_neg': 0.5 + 1.0 * tf.nn.sigmoid(student_t_params[..., 3:4]),
            'calibration_sensitivity': tf.nn.sigmoid(student_t_params[..., 4:5]),
            'tail_weight_pos': 1.0 + 3.0 * tf.nn.sigmoid(student_t_params[..., 5:6]),   # макс 4.0
    ...
    def _student_t_update(
        ...
        asymmetry_pos_scalar = tf.clip_by_value(asymmetry_pos_scalar, 0.5, 2.0)   # расширяем до полного диапазона из LSTM
        asymmetry_neg_scalar = tf.clip_by_value(asymmetry_neg_scalar, 0.5, 2.0)
        ...
        correction_factor = tf.clip_by_value(correction_factor, 1.0, 5.0)   # повышаем лимит
    ...
    def _apply_inflation_limits(self, inflation_factor, steps_above_threshold,
        ...
        inflation_factor = tf.where(
            reset_mask,
            inflation_factor * 0.5,  # Уменьшить на 50%, не сбрасывать полностью
            inflation_factor
        )  # [B]
    ...
    def adaptive_ukf_filter(
        ...
        initial_volatility: Optional[tf.Tensor] = None,
        initial_innov_window: Optional[tf.Tensor] = None,          # ← новый параметр
        anomaly_buffer_input: Optional[tf.Tensor] = None,
        buffer_index_input: Optional[tf.Tensor] = None
        ...
        B = tf.shape(Xbatch)[0]
        state_dim = self.state_dim
        innov_window_size = self.innov_window_size  # Размер окна для истории инноваций

        # Инициализация буфера (тензоры, не переменные!)
        if anomaly_buffer_input is not None:
            anomaly_buffer = tf.convert_to_tensor(anomaly_buffer_input, dtype=tf.float32)
            buffer_index = tf.convert_to_tensor(buffer_index_input, dtype=tf.int32)
        else:
            anomaly_buffer = tf.zeros([self.anomaly_buffer_size], dtype=tf.float32)
            buffer_index = tf.constant(0, dtype=tf.int32)
        ...
        # Используем уже инициализированные индексы признаков из __init__
        ...
        if initial_innov_window is not None:
            innov_window_init = tf.reshape(initial_innov_window, [B, innov_window_size])
        else:
            initial_std = 0.1
        ...
        def body(t, state, cov, vol, innov_win, inf_factor, rem_steps, last_anom_time,
                 anomaly_buffer, buffer_index,                               # ← ДОБАВЛЕНО
                 s_hist, i_hist, v_hist, f_hist, high_infl_steps, corr_adapt_hist):
            """
            ...
            vol_short_idx = self.feature_indices.get('log_vol_short', tf.constant(2, dtype=tf.int32))
            rel_vol_short_long_idx = self.feature_indices.get('rel_vol_short_long', tf.constant(18, dtype=tf.int32))
            vol_accel_idx = self.feature_indices.get('vol_accel_rel', tf.constant(19, dtype=tf.int32))
            rel_entropy_idx = self.feature_indices.get('rel_entropy', tf.constant(20, dtype=tf.int32))
            ...
            vol_medium_est = tf.abs(tf.gather(current_features, self.feature_indices.get('yz', tf.constant(8, dtype=tf.int32)), axis=1))  # [B]
            vol_long_est = tf.abs(tf.gather(current_features, self.feature_indices.get('ht', tf.constant(12, dtype=tf.int32)), axis=1))  # [B]
            ...
            buffer_pos = tf.math.mod(buffer_index, self.anomaly_buffer_size)
            # Обновляем anomaly_buffer с помощью тензорной операции
            # ✅ ИСПРАВЛЕНО: атомарное обновление в одной операции
            def update_buffer(buffer, index, value, buffer_size):
                pos = tf.math.mod(index, buffer_size)
                new_buffer = tf.tensor_scatter_nd_update(buffer, [[pos]], [value])
                return new_buffer, index + 1

            anomaly_buffer, buffer_index = update_buffer(
                anomaly_buffer, buffer_index, current_anomaly_rate, self.anomaly_buffer_size
            )
            ...
            # Используем тот же фактор (уже ограниченный в _apply_inflation_limits)
            inflation_factor_for_R = tf.reshape(inflation_factor_flat, [B_batch, 1, 1])

            # Опциональное ослабление со временем (можно оставить или убрать)
            time_penalty = tf.exp(-0.05 * tf.cast(t, tf.float32))
            ...
            R_inflated = tf.clip_by_value(R_inflated, 1e-8, 8.0)
            ...
            current_volatility = tf.reduce_mean(tf.abs(tf.gather(Xbatch, self.feature_indices['log_vol_short'], axis=2)), axis=1)
            ...
            return (
                ...
                new_inf_factor,
                new_rem_steps,
                last_anom_time_updated,
                anomaly_buffer,      # ← обновлённый буфер
                buffer_index,        # ← обновлённый индекс
                s_hist,
                i_hist,
                v_hist,
                f_hist,
        ...
        loop_vars = [
            ...
            innov_window_init,                          # innov_win
            inflation_factor_init,                       # inf_factor
            remaining_steps_init,                         # rem_steps
            last_anomaly_time_init,                        # last_anom_time
            anomaly_buffer,      # ← ДОБАВЛЕНО: текущее состояние буфера
            buffer_index,        # ← ДОБАВЛЕНО: текущий индекс буфера
            states_hist,
            innovations_hist,
            volatility_levels,
        ...
        (_, final_state, final_covariance, final_volatility, final_innov_window,
         ...
         final_anomaly_buffer, final_buffer_index,
        ...
        result = (
            ...
            tf.expand_dims(volatility_out, axis=-1),       # 2
            tf.expand_dims(inflation_factors_out, axis=-1),# 3
            final_state,                                    # 4
            final_covariance,                               # 5
            tf.expand_dims(correction_adaptive_out, axis=-1), # 6
            final_anomaly_buffer,                           # 7
            final_buffer_index,                             # 8
            final_inflation_factor,                         # 9 (уже есть в loop_vars)
            final_remaining_steps,                          # 10
            final_last_anom_time,                           # 11
            final_innov_window,                             # 12
            final_high_inflation_steps                      # 13
        )
        return result
    ...
    def compute_loss(
        ...
        if hasattr(self, "regime_selector") and self.regime_selector is not None:
            ...
            # ✅ ДОБАВЛЕНО: штраф за близость центров режимов
            separation_loss = self.regime_selector.get_center_separation_loss()
            selector_reg = selector_reg + 0.05 * separation_loss
        ...
        if hasattr(self, "regime_selector") and self.regime_selector is not None:
            ...
            entropy_penalty = tf.constant(2.0, tf.float32) * tf.square(entropy_deviation)

        # 6) Total loss (упрощённый)
        if hasattr(self, 'entropy_regularizer') and self.entropy_regularizer is not None:
            entropy_loss_val = self.lambda_entropy * entropy_loss
        else:
            entropy_loss_val = tf.constant(0.0, dtype=tf.float32)
    ...
    def compute_target_coverage(self, volatility_level: tf.Tensor) -> tf.Tensor:
        """Единая точка вычисления целевого покрытия — делегирует get_target_coverage"""
        regime_info = self.regime_selector.assign_soft_regimes(volatility_level)
        return self.get_target_coverage(volatility_level, regime_info['soft_weights'])

    def get_target_coverage(self, volatility_level, regime_soft_weights=None):
        """
        Единый метод для вычисления целевого покрытия.
        """
        base_confidence = 0.88
        confidence_range = 0.10
        confidence_ceil = base_confidence + confidence_range / 2.0
        confidence_floor = base_confidence - confidence_range / 2.0
        v_clipped = tf.clip_by_value(volatility_level, 0.0, 1.0)
        target_base = confidence_ceil - (confidence_ceil - confidence_floor) * v_clipped

        if regime_soft_weights is not None:
            target_cov_values = confidence_floor + (confidence_ceil - confidence_floor) * tf.sigmoid(self.target_cov_logits)
            target_mode = tf.reduce_sum(regime_soft_weights * target_cov_values[tf.newaxis, :], axis=1)
            alpha = tf.sigmoid(self.coverage_mixing_alpha)
            target = alpha * target_mode + (1 - alpha) * target_base
        else:
            target = target_base

        return tf.clip_by_value(target, confidence_floor, confidence_ceil)

    def _get_calibration_params(self, volatility_level, student_t_config=None, correction_adaptive=None, training=False, regime_info=None):
        """
        ...
        target_coverage = self.get_target_coverage(v, regime_info['soft_weights'])
    ...
    def _compute_calibration_loss(
        ...
        """
        ci_lower, ci_upper, y_target: [B] – только последний прогнозируемый шаг
        y_for_filtering: [B, T] – полная последовательность для оценки волатильности
        volatility_levels: [B, T, 1] – уровни волатильности за всю последовательность
        target_coverage: [B] – индивидуальные цели покрытия
        """
        B = tf.shape(y_target)[0]

        # Волатильность данных для масштабирования ширины интервалов
        ...
        y_std_batch = tf.reduce_mean(vol)                                # скаляр

        # Дифференцируемое покрытие (суррогат)
        margin = tf.maximum(0.05 * y_std_batch, 1e-2)   # минимальный порог 0.01
        soft_lower = tf.sigmoid((y_target - ci_lower) / margin)
        soft_upper = tf.sigmoid((ci_upper - y_target) / margin)
        soft_cov = soft_lower * soft_upper
        actual_coverage = tf.reduce_mean(soft_cov)                       # скаляр

        # Жёсткое покрытие (для per‑regime потерь)
        covered = tf.cast((y_target >= ci_lower) & (y_target <= ci_upper), tf.float32)  # [B]

        # Целевое покрытие (среднее по батчу)
        target_coverage_mean = tf.reduce_mean(target_coverage)           # скаляр

        # Асимметричный штраф за недопокрытие / перекрытие
        ...
        # Per‑regime калибровочная потеря
        regime_coverage_loss = tf.constant(0.0, dtype=tf.float32)
        if regime_info is not None and 'soft_weights' in regime_info:
            soft_weights_full = regime_info['soft_weights']               # [B, K] (последний шаг)
            # Взвешенное покрытие по режимам
            weighted_coverage = tf.reduce_sum(
                soft_weights_full * tf.expand_dims(covered, axis=1), axis=0
            )                                                              # [K]
            regime_coverage = weighted_coverage / (
                tf.reduce_sum(soft_weights_full, axis=0) + 1e-8
            )                                                              # [K]

            # Целевое покрытие, усреднённое по режимам
            weighted_target = tf.reduce_sum(
                soft_weights_full * tf.expand_dims(target_coverage, axis=1), axis=0
            )                                                              # [K]
            target_by_regime = weighted_target / (tf.reduce_sum(soft_weights_full, axis=0) + 1e-8)  # [K]

            regime_gap = tf.abs(regime_coverage - target_by_regime)       # [K]
        ...
        # Диагностика ширины интервала
        width = tf.maximum(ci_upper - ci_lower, 0.0)                      # [B]
        width_ratio = tf.reduce_mean(width) / (y_std_batch + 1e-8)        # скаляр

        # Целевое отношение ширины (зависит от средней волатильности)
    ...
    def _calibrate_confidence_interval(
        ...
        soft_weights: Optional[tf.Tensor] = None  # ← ДОБАВЛЕНО для дифференцируемой калибровки
    ) -> Tuple[tf.Tensor, tf.Tensor, tf.Tensor, tf.Tensor]:
        """
        Вычисляет адаптивные доверительные интервалы (CI) с режимно-зависимым контролем ширины.
        ✅ ИСПРАВЛЕНО: log-стабильность, shape consistency, защита от NaN, soft_weights
        """
        ...
        dof_floor = 6.0 - 2.0 * v
        dof_floor = tf.clip_by_value(dof_floor, 2.5, 8.0)
        ...
        if innovations is not None:
            ...
            center_shift = tf.clip_by_value(stddev * asym_ratio * 0.5, -0.5 * stddev, 0.5 * stddev)
        ...
        if hasattr(self, "max_width_factors_logits") and self.max_width_factors_logits is not None:
            ...
            # ✅ Мягкое взвешивание через soft_weights (дифференцируемо)
            if soft_weights is not None:
                target_width_per_sample = tf.reduce_sum(
                    soft_weights * factors_raw[tf.newaxis, :], axis=1
                )  # [B] - градиент проходит через regime_selector
            else:
                # fallback только если soft_weights действительно нет
    ...
    def train_step(
        ...
        y_target_batch,
        regime_labels_batch=None,          # добавлен необязательный параметр
        initial_state=None,
        ...
        ✅ ИСПРАВЛЕНО:
        - loss переменная инициализируется перед использованием
        - Все tf.cond возвращают одинаковые типы
        - Shape consistency для всех тензоров
        - soft_weights передается для дифференцируемой калибровки
        - Явная скаляризация всех метрик
        """
        B = tf.shape(X_batch)[0]

        # --- Обработка состояния из словаря (train_step) ---
        if initial_state_dict is not None:
            ...
            initial_variance = tf.clip_by_value(
                window_std ** 2,
                self._init_cov_floor,
                self._init_cov_ceil
            )
        ...
        with tf.device(self.device):
            with tf.GradientTape(persistent=False) as tape:
                ...
                params_output = lstm_outputs["params"]
                h_lstm2 = lstm_outputs["h_lstm2"]

                # 2) ENTROPY REG
                entropy_loss = self.entropy_regularizer.compute_entropy_loss(h_lstm2)

                # 3) PROCESS LSTM OUTPUTS
                ...
                # 5) EXPLICIT ONE-STEP PREDICT
                final_volatility = tf.reshape(volatility_levels[:, -1, :], [-1])
                ...
                q_base_final = tf.gather(ukf_params["q_base"], t_last, axis=1)
                ...
                inf_factor_final = tf.gather(inflation_factors, t_last, axis=1)
                correction_adaptive = correction_adaptive_hist[:, -1, :]

                # --- soft clustering regime classification ---
                regime_info = self.regime_selector.assign_soft_regimes(final_volatility)
                soft_weights = regime_info["soft_weights"]
                logits = regime_info["logits"]
                regime_assignment = regime_info.get("regime_assignment", None)

                # Cluster loss
                centers = regime_info["centers"]
                vol_flat = tf.reshape(final_volatility, [-1, 1])
                centers_flat = tf.reshape(centers, [1, -1])
                distances = tf.square(vol_flat - centers_flat)
                cluster_loss = tf.reduce_mean(tf.reduce_sum(soft_weights * distances, axis=1))

                regime_loss = 0.01 * cluster_loss   # separation_loss убрано (учитывается в compute_loss)
                ...
                # 8) CALIBRATION LOSS
                base_calibration_weight = 0.7
                warmup_steps = 400
                progress = tf.cast(self._step_counter, tf.float32) / tf.cast(warmup_steps, tf.float32)
                progress = tf.clip_by_value(progress, 0.0, 1.0)
                current_weight = base_calibration_weight * (1.0 - tf.exp(-2.0 * progress))

                # ✅ tf.cond с одинаковыми типами возврата
                should_print = tf.equal(tf.math.floormod(self._step_counter, 50), 0)
                def _print_dbg():
                    tf.print('[WARMUP DBG] step=', self._step_counter, '| progress=', progress)
                    return tf.constant(0)
                def _no_print():
                    return tf.constant(0)
                tf.cond(should_print, _print_dbg, _no_print)
                ...
                raw_calibration_loss, actual_coverage, width_ratio, target_width_ratio, width_error = \
                    self._compute_calibration_loss(
                        ci_min, ci_max,
                        y_target_batch,                        # теперь просто [B]
                        y_for_filtering_batch,
                        volatility_levels,
                        target_coverage,
                        regime_info=regime_info
                ...
                # Width loss - БЕЗ двойного учета width_regularization
                 # Width loss - ✅ ЕДИНЫЙ источник width penalty через _compute_calibration_loss
                width_warmup_steps = tf.constant(150.0, tf.float32)
                wprog = tf.cast(self._step_counter, tf.float32) / width_warmup_steps
                wprog = tf.clip_by_value(wprog, 0.0, 1.0)
                lambda_width_base = tf.constant(0.25, tf.float32)

                # Calibration loss clipping
                # ✅ ИСПРАВЛЕНО: soft minimum для избежания точек nondifferentiability
                def soft_minimum(x, threshold, temperature=0.1):
                    return threshold - tf.nn.softplus((threshold - x) / temperature) * temperature

                # Calibration loss clipping
                max_calib_weight = 0.5 * mse_loss_for_normalization
                x = current_weight * raw_calibration_loss
                calibration_loss_clipped = soft_minimum(x, max_calib_weight, temperature=0.05)

                # Train/val gap penalty — ✅ ИСПРАВЛЕНО: используем EMA от train coverage
                gap_penalty = tf.constant(0.0, tf.float32)
                train_val_gap = tf.constant(0.0, tf.float32)

                # ✅ Используем EMA от train coverage вместо val (переменная инициализирована в __init__)
                train_coverage_ema = 0.9 * self._train_coverage_ema + 0.1 * actual_coverage
                self._train_coverage_ema.assign(train_coverage_ema)
                train_val_gap = tf.abs(actual_coverage - train_coverage_ema)
                ...
                loss = self.compute_loss(
                    ...
                    inflation_factors,
                    ukf_params,
                    calibration_loss_clipped, # ← ✅ width_error уже внутри через raw_calibration_loss
                    entropy_loss,
                    regime_info=regime_info
                )

                # Regime scales regularization
                ...
                target_ratios = tf.constant([1.5, 1.33], dtype=tf.float32)
                ratio_reg = 0.01 * tf.reduce_mean(tf.square(scale_ratios - target_ratios))
                l2_reg = 1e-5 * tf.reduce_sum(tf.square(scales))
                scale_reg = ratio_reg + l2_reg

                # L2 regularization for max_width_factors_logits
                ...
                if hasattr(self, "regime_calibration_weights") and isinstance(self.regime_calibration_weights, tf.Variable):
                    trainable_vars.append(self.regime_calibration_weights)
                ...
                # ⬇️ ВСТАВИТЬ ЗДЕСЬ ⬇️
                # Отладка градиента d_raw
                if tf.equal(tf.math.floormod(self._step_counter, 50), 0):
                    for var, grad in zip(trainable_vars, gradients):
                        if 'd_raw' in var.name:
                            tf.print("d_raw gradient:", grad, " var value:", var)
                # ⬆️ КОНЕЦ ВСТАВКИ ⬆️

                # Разделение переменных по группам
                main_pairs = []
                regime_pairs = []
                temp_pairs = []
                bias_pairs = []
                ...
                # ✅ ИСПРАВЛЕНО: вычисляем inflation_anomaly_ratio
                inflation_anomaly_ratio = tf.reduce_mean(
                tf.cast(final_inflation_factor > 2.0, tf.float32)
                )

                spectrum_info = self.diff_ukf_component.get_spectrum_info()
                min_eigenvalue = spectrum_info["min_eigenvalue"]

                # ✅ ИСПРАВЛЕНО: global_norm из clip_by_global_norm
                global_norm = main_global_norm  # из предыдущего блока
                ...
                metrics = {
                    ...
                    "global_norm": global_norm,  # ← ИСПРАВЛЕНО
                    ...
                    "inflation_anomaly_ratio": inflation_anomaly_ratio,  # ← ИСПРАВЛЕНО
                    "ukf_min_eigenvalue": tf.reduce_mean(min_eigenvalue),
                    "coverage_ratio": tf.reduce_mean(actual_coverage),
                    "train_val_coverage_gap": tf.reduce_mean(train_val_gap),
                    "gap_penalty": tf.reduce_mean(gap_penalty),
                    "target_coverage": tf.reduce_mean(target_coverage_mean),
                    "ci_width_vs_stddev": tf.reduce_mean(width_ratio),
                    "calibration_error": tf.reduce_mean(tf.abs(actual_coverage - target_coverage_mean)),
                    "calib_weight": tf.reduce_mean(current_weight),
                    "calib_clipped": tf.reduce_mean(calibration_loss_clipped),
                    "calib_raw": tf.reduce_mean(raw_calibration_loss),
                    "calib_norm": tf.reduce_mean(raw_calibration_loss),  # ← ИСПРАВЛЕНО (алиас)
                    "target_width_ratio": tf.reduce_mean(target_width_ratio),
                    "width_error": tf.reduce_mean(width_error),
                    "width_loss": tf.constant(0.0),  # ← ИСПРАВЛЕНО (удалён)
                    "lambda_width": tf.constant(0.0),  # ← ИСПРАВЛЕНО (не используется)
                    "width_loss_base": tf.constant(0.0),
                    "width_loss_high_regime": tf.constant(0.0),  # ← ИСПРАВЛЕНО (удалён)
                    "width_penalty": tf.reduce_mean(width_penalty),   # актуальное значение
                }
    ...
    def _fallback_inflation_correction(
        ...
        vol_current = tf.reshape(vol_current, [-1])  # Гарантия [B]
    ...
    def val_step(
        ...
        - Все tf.cond возвращают одинаковые типы
        - Shape consistency для всех тензоров
        - Явная скаляризация метрик
        """
        B = tf.shape(X_batch)[0]

        # --- Обработка состояния из словаря (val_step) ---
        if initial_state_dict is not None:
            initial_state = initial_state_dict['state']
            initial_covariance = initial_state_dict['cov']
            initial_volatility = initial_state_dict['volatility']
        ...
        with tf.device(self.device):
            # 1) LSTM forward pass (training=False)
            ...
            # 6) Доверительные интервалы ✅ soft_weights передан
            student_t_config, target_coverage, regime_info = self._get_calibration_params(
                final_volatility,
                student_t_config=student_t_config,
                correction_adaptive=correction_adaptive
            ...
            base_calibration_weight = 0.7
            warmup_steps = 400
            progress = tf.cast(self._step_counter, tf.float32) / tf.cast(warmup_steps, tf.float32)  # ← ИСПРАВЛЕНО
            progress = tf.clip_by_value(progress, 0.0, 1.0)
            current_weight = base_calibration_weight * (1.0 - tf.exp(-2.0 * progress))
            ...
            # используем soft_minimum для дифференцируемости
            max_calib_weight = 0.5 * mse_loss_for_normalization
            x = current_weight * raw_calibration_loss
            calibration_loss_clipped = soft_minimum(x, max_calib_weight, temperature=0.05)
            calibration_loss = calibration_loss_clipped
            ...
            metrics = {
                ...
                'mse_loss': tf.reduce_mean(mse_loss),
                'entropy_loss': tf.reduce_mean(entropy_loss) if entropy_loss.shape.rank > 0 else entropy_loss,
                'width_loss': tf.reduce_mean(current_weight * tf.cast(width_error, tf.float32)),
                'coverage_ratio': tf.reduce_mean(actual_coverage),
                'target_coverage': tf.reduce_mean(target_coverage_mean),
                'ci_width_vs_stddev': tf.reduce_mean(width_ratio),
                'target_width_ratio': tf.reduce_mean(target_width_ratio),
        ...
        with tf.device(self.device):
            ...
            if epoch < warmupepochs:
                ...
                # Cosine warmup
                    ...
                    return minlr + 0.5 * (baselr - minlr) * (1.0 - tf.cos(math.pi * progress))
            else:
    ...
    def fit(self, train_features, val_features, epochs=50, min_epochs=5,
        ...
        # Вынесено за пределы цикла эпох
        state_dict = None
        ...
        try:
            for epoch in range(epochs):
                ...
                # Сброс состояния детектора аномалий перед эпохой
                self._reset_anomaly_detector_state()
                ...
                for batch_idx, (X_batch, y_for_filtering_batch, y_target_batch, regime_labels_batch) in enumerate(train_ds):
                    ...
                    # сохраняем состояние для следующего батча/эпохи
                    state_dict = results[-1]
    ...
    def inverse_transform_std(self, std_scaled: np.ndarray, forecast_scaled: np.ndarray) -> np.ndarray:
        """
        Корректное обратное преобразования std для нелинейных скейлеров.
        Использует локальный градиент преобразования.
        """
        if self.feature_scalers is None or 'Y' not in self.feature_scalers:
            return std_scaled
        ...
        if hasattr(y_scaler, 'scale_'):  # StandardScaler, MinMaxScaler
            return std_scaled * y_scaler.scale_

        elif hasattr(y_scaler, 'lambdas_'):  # PowerTransformer
            # Локальная линейная аппроксимация через градиент
            forecast_mean = float(np.mean(forecast_scaled.flatten()))
            eps = 1e-4

            try:
                upper = y_scaler.inverse_transform([[forecast_mean + eps]])[0, 0]
                lower = y_scaler.inverse_transform([[forecast_mean - eps]])[0, 0]
                local_scale = (upper - lower) / (2 * eps)
                return std_scaled * local_scale
            except Exception:
                # Fallback: использовать среднее значение лямбды
                lambda_val = np.mean(y_scaler.lambdas_)
                if lambda_val > 0:
                    return std_scaled * (forecast_mean ** (1 - lambda_val) + 1e-8)
                else:
                    return std_scaled * 0.5  # Консервативная оценка
    ...
    def save(self, path: str):
        ...
        model_state = {
            ...
            # Добавлено: состояние детектора (inflation factor, remaining steps, high inflation steps, innov window)
            'last_inflation_factor': float(self._last_inflation_factor.numpy().reshape(-1)[0]) if hasattr(self, '_last_inflation_factor') else 1.0,
            'last_remaining_steps': int(self._last_remaining_steps.numpy().reshape(-1)[0]) if hasattr(self, '_last_remaining_steps') else 0,
            'last_high_inflation_steps': int(self._last_high_inflation_steps.numpy().reshape(-1)[0]) if hasattr(self, '_last_high_inflation_steps') else 0,
            'last_innov_window': self._last_innov_window.numpy().tolist() if hasattr(self, '_last_innov_window') else None,
    ...
    def load(self, path: str):
        ...
        # Восстановление новых переменных состояния детектора
        if 'last_inflation_factor' in model_state and hasattr(self, '_last_inflation_factor'):
            self._last_inflation_factor.assign([model_state['last_inflation_factor']])
        if 'last_remaining_steps' in model_state and hasattr(self, '_last_remaining_steps'):
            self._last_remaining_steps.assign([model_state['last_remaining_steps']])
        if 'last_high_inflation_steps' in model_state and hasattr(self, '_last_high_inflation_steps'):
            self._last_high_inflation_steps.assign([model_state['last_high_inflation_steps']])
        if 'last_innov_window' in model_state and model_state['last_innov_window'] is not None and hasattr(self, '_last_innov_window'):
            self._last_innov_window.assign(np.array(model_state['last_innov_window'], dtype=np.float32))

        # === 6) restore scalar params ===
        scalar_params = ['max_width_factors_logits', 'lambda_entropy']
    ...
    def get_current_weights(self) -> Dict[str, Any]:
        ...
        state_dict: Dict[str, Any] = {
            ...
            # Новые переменные состояния детектора
            "_last_inflation_factor": float(self._last_inflation_factor.numpy().reshape(-1)[0]) if hasattr(self, '_last_inflation_factor') else 1.0,
            "_last_remaining_steps": int(self._last_remaining_steps.numpy().reshape(-1)[0]) if hasattr(self, '_last_remaining_steps') else 0,
            "_last_high_inflation_steps": int(self._last_high_inflation_steps.numpy().reshape(-1)[0]) if hasattr(self, '_last_high_inflation_steps') else 0,
            "_last_innov_window": self._last_innov_window.numpy().tolist() if hasattr(self, '_last_innov_window') else None,
    ...
    def load_best_weights(self) -> bool:
        ...
        try:
            ...
            # Восстановление новых переменных из best_weights_dict
            if "_last_inflation_factor" in d and d["_last_inflation_factor"] is not None and hasattr(self, "_last_inflation_factor"):
                try:
                    self._last_inflation_factor.assign([d["_last_inflation_factor"]])
            ...
            self._reset_anomaly_detector_state()
            print("   ✅ Детектор аномалий сброшен")
    ...
    def reset_best_weights_tracking(self):
        ...
        if hasattr(self, '_prev_val_coverage'):
            self._prev_val_coverage.assign(tf.constant(0.89, dtype=tf.float32))

    def _reset_anomaly_detector_state(self):
        """Сброс всех накопительных переменных детектора аномалий."""
        if hasattr(self, 'anomaly_buffer'):
            self.anomaly_buffer.assign(tf.zeros_like(self.anomaly_buffer))
        if hasattr(self, 'buffer_index'):
            self.buffer_index.assign(0)
        if hasattr(self, '_last_anomaly_time'):
            self._last_anomaly_time.assign(tf.fill(tf.shape(self._last_anomaly_time), -100))
        if hasattr(self, '_last_inflation_factor'):
            self._last_inflation_factor.assign([1.0])
        ...
        if hasattr(self, '_last_innov_window'):
            innov_window_size = getattr(self, 'innov_window_size', 20)
            self._last_innov_window.assign(tf.zeros([1, innov_window_size]))
        ...
        # Сброс новых переменных детектора
        if hasattr(self, '_last_inflation_factor'):
            try:
                self._last_inflation_factor.assign([1.0])
        ...
        if hasattr(self, '_last_innov_window'):
            try:
                innov_window_size = getattr(self, 'innov_window_size', 20)
                self._last_innov_window.assign(tf.zeros([1, innov_window_size]))
                print("   ✅ _last_innov_window сброшен")
            except Exception as e:
                print(f"   ⚠️ Не удалось сбросить _last_innov_window: {str(e)}")
    ...
    def _online_predict_step(
        ...
        anomaly_buffer_input: Optional[tf.Tensor] = None,
        buffer_index_input: Optional[tf.Tensor] = None,
        last_inflation_factor: Optional[tf.Tensor] = None,
        last_remaining_steps: Optional[tf.Tensor] = None,
        last_high_inflation_steps: Optional[tf.Tensor] = None,
        last_innov_window: Optional[tf.Tensor] = None
    ) -> Tuple[
        tf.Tensor, tf.Tensor, tf.Tensor, tf.Tensor, tf.Tensor,
        tf.Tensor, tf.Tensor, tf.Tensor, tf.Tensor, Dict[str, tf.Tensor],
        tf.Tensor, tf.Tensor  # ← НОВЫЕ: anomaly_buffer, buffer_index
    ]:
        ...
        # last_volatility: гарантированное приведение к [B]
        last_volatility = tf.convert_to_tensor(last_volatility, dtype=tf.float32)
        last_volatility = tf.reshape(tf.squeeze(last_volatility), [B])
        last_volatility = tf.ensure_shape(last_volatility, [B])
        ...
        results = self.adaptive_ukf_filter(
            ...
            initial_innov_window=last_innov_window,          # ← передаём сохранённое окно
            anomaly_buffer_input=anomaly_buffer_input,
            buffer_index_input=buffer_index_input
        )
        ...
        # 🔧 ГАРАНТИРОВАННОЕ ПОЛУЧЕНИЕ regime_assignment
        regime_assignment = regime_info.get("regime_assignment")
        if regime_assignment is None:
            # fallback через argmax от soft_weights
            regime_assignment = tf.argmax(regime_info['soft_weights'], axis=1, output_type=tf.int32)
        ...
        return (
            ...
            target_coverage,
            regime_info,
            updated_anomaly_buffer,
            updated_buffer_index,
            final_inflation_factor,      # ← новое
            final_remaining_steps,       # ← новое
            final_last_anom_time,        # ← новое
            final_innov_window,          # ← новое
            final_high_inflation_steps   # ← новое
        )
    ...
    def online_predict(
        ...
        # Добавляем новые переменные состояния детектора
        if not hasattr(self, "_last_inflation_factor"):
            self._last_inflation_factor = tf.Variable([1.0], trainable=False, dtype=tf.float32, name="last_inflation_factor")
        if not hasattr(self, "_last_remaining_steps"):
            self._last_remaining_steps = tf.Variable([0], trainable=False, dtype=tf.int32, name="last_remaining_steps")
        if not hasattr(self, "_last_high_inflation_steps"):
            self._last_high_inflation_steps = tf.Variable([0], trainable=False, dtype=tf.int32, name="last_high_inflation_steps")
        if not hasattr(self, "_last_innov_window"):
            innov_window_size = self.innov_window_size
        ...
        if reset_state or (not bool(self._state_initialized.numpy())):
            ...
            initial_variance = max(window_std ** 2, float(self._init_cov_floor))
            initial_variance = min(initial_variance, float(self._init_cov_ceil))
        ...
        (
            ...
            final_last_anom_time,
            final_innov_window,
            final_high_inflation_steps
        ) = self._online_predict_step(
            X_scaled_tensor,
            initial_state,
            initial_covariance,
            initial_volatility,
            anomaly_buffer_input=self.anomaly_buffer.value(),
            buffer_index_input=self.buffer_index.value(),
            last_inflation_factor=self._last_inflation_factor if hasattr(self, '_last_inflation_factor') else None,
            last_remaining_steps=self._last_remaining_steps if hasattr(self, '_last_remaining_steps') else None,
            last_high_inflation_steps=self._last_high_inflation_steps if hasattr(self, '_last_high_inflation_steps') else None,
        ...
        if self.feature_scalers is not None and "Y" in self.feature_scalers:
            ...
            std_dev_original = self.inverse_transform_std(
                std_dev_scaled.numpy(),
    ...
    def evaluate(
        ...
        # === ВСТАВИТЬ ЗДЕСЬ ===
        original_state = {
            'last_volatility': self._last_volatility.numpy().copy() if hasattr(self, '_last_volatility') else None,
            'anomaly_buffer': self.anomaly_buffer.numpy().copy() if hasattr(self, 'anomaly_buffer') else None,
            'buffer_index': int(self.buffer_index.numpy()) if hasattr(self, 'buffer_index') else 0,
            'last_anomaly_time': self._last_anomaly_time.numpy().copy() if hasattr(self, '_last_anomaly_time') else None,
            'last_inflation_factor': self._last_inflation_factor.numpy().copy() if hasattr(self, '_last_inflation_factor') else None,
            'last_remaining_steps': self._last_remaining_steps.numpy().copy() if hasattr(self, '_last_remaining_steps') else None,
            'last_high_inflation_steps': self._last_high_inflation_steps.numpy().copy() if hasattr(self, '_last_high_inflation_steps') else None,
            'last_innov_window': self._last_innov_window.numpy().copy() if hasattr(self, '_last_innov_window') else None,
        }
        ...
        try:
            ...
            if original_state_initialized_val:
                ...
                # 🔑 ВОССТАНОВЛЕНИЕ ВСЕХ ПЕРЕМЕННЫХ ДЕТЕКТОРА
                if hasattr(self, '_last_volatility') and 'last_volatility' in original_state:
                    self._last_volatility.assign(original_state['last_volatility'])
                if hasattr(self, 'anomaly_buffer') and 'anomaly_buffer' in original_state:
                    self.anomaly_buffer.assign(original_state['anomaly_buffer'])
                if hasattr(self, 'buffer_index') and 'buffer_index' in original_state:
                    self.buffer_index.assign(original_state['buffer_index'])
                if hasattr(self, '_last_anomaly_time') and 'last_anomaly_time' in original_state:
                    self._last_anomaly_time.assign(original_state['last_anomaly_time'])
                if hasattr(self, '_last_inflation_factor') and 'last_inflation_factor' in original_state:
                    self._last_inflation_factor.assign(original_state['last_inflation_factor'])
                if hasattr(self, '_last_remaining_steps') and 'last_remaining_steps' in original_state:
                    self._last_remaining_steps.assign(original_state['last_remaining_steps'])
                if hasattr(self, '_last_high_inflation_steps') and 'last_high_inflation_steps' in original_state:
                    self._last_high_inflation_steps.assign(original_state['last_high_inflation_steps'])
                if hasattr(self, '_last_innov_window') and 'last_innov_window' in original_state:
                    self._last_innov_window.assign(original_state['last_innov_window'])
            else:
                self._state_initialized.assign(False)
    ...
    def _plot_evaluation_results(
        ...
        inflation_mean = np.mean(inflation_vals)
        ax4.set_title(f'Adaptive inflation (среднее: {inflation_mean:.4f})',
        ...
        confidence_mean = np.mean(confidence_vals)
        ax6.set_title(f'Уровень уверенности прогнозов\n(среднее: {confidence_mean:.4f})',
                      fontsize=7, fontweight='bold')
